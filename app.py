...
def is_student_in_poverty(student_data):
    """判断学生是否为特别贫困"""
    yearly_poverty_data = extract_yearly_poverty_level_data(student_data)
    
    # 检查所有学年的贫困等级
    for year_num_str, raw_value in yearly_poverty_data.items():
        if raw_value and not pd.isna(raw_value):
            formatted_value = format_value(raw_value)
            if formatted_value != '无':
                return True
    
    # 如果没有学年数据，检查通用贫困等级列
    poverty_columns = ['贫困等级', '困难等级', '家庭经济困难等级', '经济困难等级']
...
def is_student_extremely_poor(student_data):
    """判断学生是否为特别困难（只筛选"特别困难"等级）"""
    yearly_poverty_data = extract_yearly_poverty_level_data(student_data)
    
    # 检查所有学年的贫困等级是否有"特别困难"
    for year_num_str, raw_value in yearly_poverty_data.items():
        if raw_value and not pd.isna(raw_value):
            if str(raw_value).strip() == '特别困难':
                return True
    
    return False

def is_student_physical_test_failed(student_data):
    """判断学生体测成绩是否不合格"""
    # 动态提取学年数据
    academic_years = extract_academic_year_data(student_data)
    
    # 检查各学年的体测成绩和体测评级
    for year_num_str, year_data in academic_years.items():
        # 检查体测成绩（数值）
        physical_score = year_data.get('体测成绩')
        if physical_score and not pd.isna(physical_score):
            try:
                score = float(physical_score)
                if score < 60:  # 60分以下为不合格
                    return True
            except (ValueError, TypeError):
                pass
        
        # 检查体测评级（文字）
        physical_rating = year_data.get('体测评级')
        if physical_rating and not pd.isna(physical_rating):
            rating_str = str(physical_rating).strip().lower()
            if rating_str in ['不合格', '不及格', '未通过', 'fail', 'failed', '差']:
                return True
    
    # 如果没有学年数据，检查通用体测列
    general_columns = ['体测成绩', '体测评级', '体质测试成绩', '体质测试等级']
    for col in general_columns:
        if col in student_data.index:
            ...
            if value and not pd.isna(value):
                ...
                # 检查评级
                if '评级' in col or '等级' in col:
                    rating_str = str(value).strip().lower()
                    if rating_str in ['不合格', '不及格', '未通过', 'fail', 'failed', '差']:
                        return True
    
    return False

def is_student_psych_level_1(student_data):
    """判断学生心理测评等级是否为一级"""
    yearly_psych_data = extract_yearly_psychological_level_data(student_data)
    
    # 检查所有学年的心理测评等级
    for year_num_str, raw_value in yearly_psych_data.items():
        if raw_value and not pd.isna(raw_value):
            psych_value = format_value(raw_value)
            if psych_value in ['1级', '1', 'I级', 'I', '一级', '较差', '差']:
                return True
    
    # 如果没有学年数据，检查通用心理评测等级
    if not yearly_psych_data:
        psychological_level = student_data.get('心理评测等级', student_data.get('最新心理等级', student_data.get('心理等级')))
        if psychological_level and not pd.isna(psychological_level):
...
def is_student_gpa_below_2(student_data):
    """判断学生平均绩点是否低于2.0"""
    # 尝试获取总绩点
    overall_gpa_value = student_data.get('总绩点', student_data.get('平均学分绩点'))
    
    if overall_gpa_value and not pd.isna(overall_gpa_value):
        try:
            gpa = float(overall_gpa_value)
            return gpa < 2.0
        except (ValueError, TypeError):
            pass
...
def is_problem_student(student_data):
    """判断学生是否为有问题学生（同时满足三个条件）"""
    return (is_student_in_poverty(student_data) and 
            is_student_psych_level_1(student_data) and 
            is_student_gpa_below_2(student_data))
...
if st.session_state.students_data is None:
    ...
    with col1:
        ...
        # 问题学生筛选选项（多选）
        st.markdown("🎯 **筛选学生类型** ")
        filter_extremely_poor = st.checkbox("特别困难学生", key="filter_extremely_poor")
        filter_psych_level1 = st.checkbox("心理测评一级学生", key="filter_psych_level1")
        filter_gpa_below2 = st.checkbox("绩点低于2.0学生", key="filter_gpa_below2")
        filter_physical_fail = st.checkbox("体测成绩不合格学生", key="filter_physical_fail")
        ...
        # 应用问题学生筛选（多条件AND逻辑）
        if any([filter_extremely_poor, filter_psych_level1, filter_gpa_below2, filter_physical_fail]):
            problem_indices = []
            for idx, row in filtered_df.iterrows():
                # 检查所有勾选条件，必须全部满足
                conditions_met = []
                
                if filter_extremely_poor:
                    conditions_met.append(is_student_extremely_poor(row))
                
                if filter_psych_level1:
                    conditions_met.append(is_student_psych_level_1(row))
                
                if filter_gpa_below2:
                    conditions_met.append(is_student_gpa_below_2(row))
                
                if filter_physical_fail:
                    conditions_met.append(is_student_physical_test_failed(row))
                
                # 所有勾选的条件都必须满足
                if conditions_met and all(conditions_met):
                    problem_indices.append(idx)
            
            filtered_df = filtered_df.loc[problem_indices]
            
            # 调试信息
            selected_filters = []
            if filter_extremely_poor:
                selected_filters.append("特别困难")
            if filter_psych_level1:
                selected_filters.append("心理测评一级")
            if filter_gpa_below2:
                selected_filters.append("绩点低于2.0")
            if filter_physical_fail:
                selected_filters.append("体测不合格")
            
            if selected_filters:
                filter_text = " + ".join(selected_filters)
                st.info(f"🔍 筛选到 {len(filtered_df)} 名同时满足条件的学生: {filter_text}")
